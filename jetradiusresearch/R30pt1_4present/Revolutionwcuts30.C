void Revolutionwcuts30(TString infile = "histos.root"){
    set_plot_style();
    gStyle->SetOptStat(0);
    ...
    for(int i=0;i<4;i++){
        c1->cd(i+1);
        array[i]->Draw("colz");
        array[i]->GetYaxis()->SetTitleSize(.05);
        array[i]->GetXaxis()->SetTitleSize(.05);
        array[i]->GetYaxis()->SetTitle("X_{J#gamma}");
        array[i]->GetXaxis()->SetTitle("Jet p_{T} (GeV/c)");
        gPad->SetLogz();
   ...
   /*TH1* negbackground[16];
    for(int i=0;i<16;i++){
        negbackground[i]=Background[i]->Scale(1);
    }//
