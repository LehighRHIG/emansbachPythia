void parton(TString infile = "histos.root"){
    set_plot_style();
    gStyle->SetOptStat(0);
    ...
    TH1* deltphione = (TH1*)gROOT->FindObject("deltphig");
    array[0]= deltphione;
    
    TFile *_file1 = TFile::Open("histtwo.root");
    TH1* deltphitwo = (TH1*)gROOT->FindObject("deltphig");
    array[1]= deltphitwo;

    TFile *_file2 = TFile::Open("histthree.root");
    TH1* deltphithree = (TH1*)gROOT->FindObject("deltphig");
    array[2]= deltphithree;
    
    TFile *_file3 = TFile::Open("histfour.root");
    TH1* deltphifour = (TH1*)gROOT->FindObject("deltphig");
    array[3]= deltphifour;
    
    TFile *_file4 = TFile::Open("histone.root");
    TH1* deltphifive = (TH1*)gROOT->FindObject("deltphiq");
    array[4]= deltphifive;

    TFile *_file5 = TFile::Open("histtwo.root");
    TH1* deltphisix = (TH1*)gROOT->FindObject("deltphiq");
    array[5]= deltphisix;
    
    TFile *_file6 = TFile::Open("histthree.root");
    TH1* deltphiseven = (TH1*)gROOT->FindObject("deltphiq");
    array[6]= deltphiseven;
    
    TFile *_file7 = TFile::Open("histfour.root");
    TH1* deltphieight = (TH1*)gROOT->FindObject("deltphiq");
    ...
    */
    ...
    for(int i=0;i<8;i=i+4){
        c1->cd(1);
        array[i]->GetYaxis()->SetRangeUser(0, .0003);
        if(i>3){
            array[i]->SetMarkerColor(kRed);
            array[i]->SetLineColor(kRed);
            array[i]->Draw("sameehist");
        ...
        TLegend* leg = new TLegend(0.3,0.7,0.4,0.85);
        SetLeg(leg,.04);
    ...
    for(int i=1;i<8;i=i+4){
        ...
        leg2->AddEntry(array[1],"gluons","l");
        leg2->AddEntry(array[5],"quarks","l");
        leg2->Draw();
    }
